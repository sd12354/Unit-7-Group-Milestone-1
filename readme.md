...
## Milestone #1
<div style="width:260px;max-width:100%;"><div style="height:0;padding-bottom:198.08%;position:relative;"><iframe width="260" height="515" style="position:absolute;top:0;left:0;width:100%;height:100%;" frameBorder="0" src="https://imgflip.com/embed/aqblo4"></iframe></div><p><a href="https://imgflip.com/gif/aqblo4">via Imgflip</a></p></div>
...
## Milestone 2 — Core session features (Unit 8 / Sprint 1)

**Status:** Implemented in the iOS app (Firestore). Maps to GitHub Issues 5–8.

| Issue | Deliverable | Status |
| ----- | ----------- | ------ |
| 5 | `StudySession` model + `SessionRepository`: `createSession`, `getSessions`, `getSessionById`, live `observeSession` | Done |
| 6 | Create tab: form (title, subject, date/time, location, max attendees, description) → Firestore → navigate to session detail | Done |
| 7 | Home feed: chronological upcoming sessions, row summary, pull-to-refresh, tap → detail | Done |
| 8 | Session detail: full fields, attendee list, Join / Leave with capacity + transactional join | Done |

### Firestore (`sessions` collection)

Documents use Swift `Codable` field names: `title`, `subjectTag`, `startTime` (Timestamp), `endTime`, `locationText`, `description`, `maxAttendees`, `hostId`, `attendeeIds` (array of UIDs).

**Project board / issues / GIFs:** Create the GitHub Project, milestones, and issues per the Unit 8 lab; record sprint-2 GIFs or screen recordings here when your group captures them.

_Add screen recording or GIF (create → feed → join/leave) below._

<!-- ![](docs/milestone2-sessions.gif) -->

**Firebase setup notes**

- Add `GoogleService-Info.plist` to the Xcode target (already gitignored).
- In Firebase Console → Authentication → enable **Anonymous** (used until Issue 2 email login ships).
- Firestore security rules must allow authenticated users to read sessions and create/update attendee arrays for your class demo (tighten rules before production).
